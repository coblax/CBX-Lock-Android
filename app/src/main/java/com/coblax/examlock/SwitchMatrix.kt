...
object GateResolver {
    ...
    @Volatile private var cachedSeed: Int? = null
    @Volatile private var cachedPermutation: IntArray? = null
...
internal object SwitchMatrix {
    private val criticalGateIds = setOf(
        ...
        GateKeys.Vpn.id,
        GateKeys.ScreenRecorder.id,
        GateKeys.DisplayMirror.id,
        GateKeys.MultiWindow.id,
        GateKeys.ReverseEngineering.id,
        GateKeys.ApkIntegrity.id
    )
...
internal object LegacySwitchMatrixStorage {
    private fun prefs(context: Context): SharedPreferences? {
        ...
        return runCatching {
            ...
            android.util.Log.e("LegacySwitchMatrix",
                "EncryptedSharedPreferences unavailable for legacy switch matrix.", it)
        }.getOrNull()
    }

    fun containsLegacyState(context: Context): Boolean {
        return prefs(context)?.all?.isNotEmpty() ?: false
    }
    ...
    fun setEnabled(context: Context, key: GateKey, enabled: Boolean) {
        val preferences = prefs(context) ?: return
        val idx = GateResolver.indexFor(context, key)
        val mask = 1L shl idx
        val bits = preferences.getLong(matrixKey(context), 0L)
